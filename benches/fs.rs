use criterion::{
    ...
    BenchmarkId, BatchSize, Throughput,
};
...
use cesiumdb::fs::{CompactionConfig, Fs};
...
fn benchmark_fragmentation(c: &mut Criterion) {
    ...
    for frag_ops in [10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(frag_ops), frag_ops, |b, &frag_ops| {
            b.iter_batched(
                || {
                    ...
                    for _ in 0..frag_ops {
                        ...
                        if let Ok(id) = fs.create_frange(1024 * 64) {  // 64KB small frange
                            ids.push(id);
                        }
                        if let Ok(id) = fs.create_frange(1024 * 1024) {  // 1MB large frange
                            ids.push(id);
                        }
                    }
            ...
            |b, &level| {
                b.iter_batched(
                    || {
                        ...
                        fs.maybe_compact_with_config(&CompactionConfig::default()).unwrap();
                        (fs, dir)
                    },
                    |(fs, _dir)| {
