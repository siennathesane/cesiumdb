...
// nb (sienna): use `Tombstone.marker` as the namespace separators. so we sort the array first by tombstone
// then by whatever their comparators are, then each tombstone is the index value of when the next tombstone
// starts. this lets us optimize a binary search into a small subset of the entire array at the cost of space
// but the space is logrithmic and only grows based on the amount of namespaces. this is sorted small on the
// left, big on the right
...
mod tests {
    ...
    fn test_insert_single_tombstone() {
        ...
        vec.insert(TestItem { marker: 1, value: 10 });
        assert_eq!(vec.buf, vec![TestItem { marker: 1, value: 10 }]);
    ...
    fn test_insert_multiple_tombstones() {
        let mut vec = TombstonedVec::new();
        vec.insert(TestItem { marker: 1, value: 10 });
        vec.insert(TestItem { marker: 2, value: 20 });
        vec.insert(TestItem { marker: 3, value: 30 });
        assert_eq!(vec.buf, vec![
            TestItem { marker: 1, value: 10 },
            TestItem { marker: 2, value: 20 },
            TestItem { marker: 3, value: 30 },
        ]);
        assert_eq!(vec.tombstones, [(1, 0), (2, 1), (3, 2)].into_iter().collect());
    }
    ...
    fn test_insert_same_tombstone() {
        let mut vec = TombstonedVec::new();
        vec.insert(TestItem { marker: 1, value: 10 });
        vec.insert(TestItem { marker: 1, value: 5 });
        vec.insert(TestItem { marker: 1, value: 15 });
        assert_eq!(vec.buf, vec![
            TestItem { marker: 1, value: 5 },
            TestItem { marker: 1, value: 10 },
            TestItem { marker: 1, value: 15 },
        ]);
        assert_eq!(vec.tombstones, [(1, 0)].into_iter().collect());
    }
    ...
    fn test_insert_mixed_order() {
        ...
        vec.insert(TestItem { marker: 1, value: 10 });
        vec.insert(TestItem { marker: 3, value: 30 });
        vec.insert(TestItem { marker: 2, value: 25 });
        vec.insert(TestItem { marker: 1, value: 15 });
        assert_eq!(vec.buf, vec![
            TestItem { marker: 1, value: 10 },
            TestItem { marker: 1, value: 15 },
            TestItem { marker: 2, value: 20 },
            TestItem { marker: 2, value: 25 },
            TestItem { marker: 3, value: 30 },
        ]);
        assert_eq!(vec.tombstones, [(1, 0), (2, 2), (3, 4)].into_iter().collect());
    }
    ...
    fn test_insert_large_gap_between_tombstones() {
        let mut vec = TombstonedVec::new();
        vec.insert(TestItem { marker: 1, value: 10 });
        vec.insert(TestItem { marker: 1000, value: 1000 });
        vec.insert(TestItem { marker: 500, value: 500 });
        assert_eq!(vec.buf, vec![
            TestItem { marker: 1, value: 10 },
            TestItem { marker: 500, value: 500 },
            TestItem { marker: 1000, value: 1000 },
        ]);
        assert_eq!(vec.tombstones, [(1, 0), (500, 1), (1000, 2)].into_iter().collect());
    }
    ...
    fn test_insert_many_items() {
        let mut vec = TombstonedVec::new();
        for i in 0..1000 {
            vec.insert(TestItem { marker: i / 100, value: i });
        }
    ...
    fn test_insert_reverse_order() {
        let mut vec = TombstonedVec::new();
        for i in (0..10).rev() {
            vec.insert(TestItem { marker: i, value: i * 10 });
        }
        assert_eq!(vec.buf, (0..10).map(|i| TestItem { marker: i, value: i * 10 }).collect::<Vec<_>>());
