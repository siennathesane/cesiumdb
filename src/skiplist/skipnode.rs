...
/// between current node and the next node. If there's no next node, the
/// distance is distance between current node and last reachable node.
///
...
impl<V> SkipNode<V> {
    ...
    /// `Option<&V>` is the value of current node (`None` if the current node is
    /// the head), `&V` is the value of the next node.
    ...
    /// Try to move for the given distance, only using links at the specified
    /// level. If it's impossible, then move as far as possible.
    ///
    ...
    ///
    /// Requries that there's nothing before the node and the new node can't be
    /// at a higher level.
    ///
...
impl<V> Drop for SkipNode<V> {
    fn drop(&mut self) {
        // SAFETY: all nodes are going to be dropped; its okay that its links (except
        // those at level 0) become dangling.
        unsafe {
...
/// A SeekListAction seeks a node on the list and do something, e.g. insertion,
/// deletion, replacement, on it, often mutating the links in the process.
///
/// Actions on skiplists usually consist of three phases: for each level, seek
/// the node on which we modify the links, actually modify the link on the 0th
/// level, and fixup the links at upper levels.
///
/// Between the phases there are some bookkeeping, and this trait abstract that
/// away.
///
/// To use this trait, just implement the 3 phases (`seek`, `act_on_node`,
/// `fixup`), and call `act()` on the given list.
///
/// For examples, see one of the types that implements this trait, such as
/// [IndexInserter] or [IndexRemover].
pub trait SkipListAction<'a, T>: Sized {
    ...
    /// In this case, the action is considered to have failed, and
    /// `Self::fail()` will be called.
    ///
    ...
    /// # SAFETY
    /// If `Self::Ok` is a reference, it shall not alias with any node that
    /// needs fixup.
    unsafe fn act_on_node(self, node: &'a mut SkipNode<T>) -> Result<Self::Ok, Self::Err>;

    /// Usually SkipListAction breaks links between nodes,  this method should
    /// fix that up.
    ///
    /// It should never fail.
    ///
    /// `level_head` is the node whose links may needs to be fixed.
    /// `action_result` is a mutable reference to the return value of
    /// act_on_node(). `distance_to_target` is distance from `level_head` to
    /// the node that has been acted on.
    fn fixup(
        level: usize,
...
impl<'a, V: 'a> SkipListAction<'a, V> for IndexInserter<V> {
    ...
    /// SAFETY: This returns a new node, which should never alias with any old
    /// nodes.
...
/// `distance_to_parent` is the distance from `level_head` to the parent of
/// `new_node`.
...
impl<V> SkipNode<V> {
    ...
    /// return a reference to the last node whose item compares less than the
    /// target, and the distance to that node.
    ...
    /// Given a list head, a comparison function and a target,
    /// return a mutable reference to the last node whose item compares less
    /// than the target. and the distance to that node.
    pub fn find_last_le_with_mut<F, T: ?Sized>(
        &mut self,
        cmp: F,
        target: &T,
    ) -> (&mut Self, usize)
    ...
    /// Given a list head, a comparison function and a target,
    /// return a reference to the last node whose item compares less than or
    /// equal to the target. and the distance to that node.
    ...
    /// Given a list head, a comparison function and a target,
    /// return a mutable refeerence to the last node whose item compares less
    /// than or equal to the target. and the distance to that node.
    #[allow(dead_code)]
    pub fn find_last_lt_with_mut<F, T: ?Sized>(
        &mut self,
        cmp: F,
        target: &T,
    ) -> (&mut Self, usize)
...
// so the members are named first and last instaed of head/end to avoid
// confusion.

/// An iterator for [SkipList] and [OrderedSkipList].
pub struct Iter<'a, T> {
...
/// A mutable iterator for [SkipList] and [OrderedSkipList].
...
impl<'a, T> Iterator for IterMut<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if ptr::eq(current_node, self.last.unwrap().as_ptr()) {
            ...
            // calling current_node.next_mut() borrows it, transforming the reference to a
            // pointer unborrows that.
            ...
            // until this function returns, and the returned reference does not points to a
            // node.
            unsafe {
                self.first = p.as_mut();
            }
...
/// Consuming iterator for [SkipList] and [OrderedSkipList].
...
impl<T> DoubleEndedIterator for IntoIter<T> {
    fn next_back(&mut self) -> Option<T> {
        ...
        let popped_node = if ptr::eq(self.first.as_deref().as_ptr(), self.last.unwrap().as_ptr()) {
            ...
            // SAFETY: we checked that self.last points to somewhere and does not alias to
            // self.first
            let new_last = unsafe { self.last.unwrap().as_mut().prev };
            if ptr::eq(self.first.as_deref().as_ptr(), new_last.unwrap().as_ptr()) {
                ...
                // SAFETY: we checked new_last points to somewhere and do not alias to
                // self.first.
